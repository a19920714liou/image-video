...
import static com.bjhit.video.codecs.H264Const.BLK_X;
import static com.bjhit.video.codecs.H264Const.BLK_Y;
import static com.bjhit.video.codecs.H264Const.MB_BLK_OFF_LEFT;
import static com.bjhit.video.codecs.H264Const.MB_BLK_OFF_TOP;
import static com.bjhit.video.codecs.H264Utils.escapeNAL;
import static com.bjhit.video.codecs.decode.CoeffTransformer.reorderDC4x4;
import static com.bjhit.video.codecs.io.model.MBType.I_16x16;
import static com.bjhit.video.common.tools.MathUtil.clip;

import java.nio.ByteBuffer;

import com.bjhit.video.codecs.decode.CoeffTransformer;
import com.bjhit.video.codecs.encode.DumbRateControl;
import com.bjhit.video.codecs.encode.RateControl;
import com.bjhit.video.codecs.io.CAVLC;
...
import com.bjhit.video.codecs.io.model.RefPicMarkingIDR;
import com.bjhit.video.codecs.io.model.SeqParameterSet;
import com.bjhit.video.codecs.io.model.SliceHeader;
import com.bjhit.video.codecs.io.model.SliceType;
import com.bjhit.video.codecs.io.write.CAVLCWriter;
import com.bjhit.video.codecs.io.write.SliceHeaderWriter;
import com.bjhit.video.common.ArrayUtil;
import com.bjhit.video.common.VideoEncoder;
 ...
 * H.264传输的码流结构：
 *        .......................................................................
 *  NAL层  |--NAL Header--|----VCL层---------------------------------------------|
 *  .............................................................................
 *  条带层或片(slice)    |条带(片)Header|-----------条带(片)数据------------------|
 *  .............................................................................
 *  宏块层(MB)                          |宏块Header|-----------宏块数据----------|
 *  .............................................................................
 *  块层(block)                                    |块Header|-----块数据---------|
 *  .............................................................................
 * 
 * @description H.264编码，目前只支持I帧和基于上下文可变长编码(CAVLC)，还不支持P、B帧和基于上下文自适应二进制算法编码(CABAC)等
 * @project bjhit-video
 * @author guanxianchun
 * @Create 2015-2-11 下午3:48:13
 * @version 1.0
 */
public class H264Encoder implements VideoEncoder {
	...
	public ByteBuffer encodeFrame(Picture pic, ByteBuffer _out, boolean idr, int poc) {
		...
		if (idr) {
			...
			writeSPS(dup, sps);//写入sps信息

			dup.putInt(0x1);
			new NALUnit(NALUnitType.PPS, 3).write(dup);
			writePPS(dup, pps);//写入pps信息
		}
	...
	private void encodeSlice(SeqParameterSet sps, PictureParameterSet pps, Picture pic, ByteBuffer dup, boolean idr, int poc) {
		...
		sh.slice_type = SliceType.I;//片头的类型为I
		if (idr)
	...
	private void writeAC(int comp, int mbX, int mbY, BitWriter out, int mbLeftBlk, int mbTopBlk, int[][] ac, int qp) {
		for (int i = 0; i < ac.length; i++) {
			...
			// TODO: calc here
			cavlc[comp].writeACBlock(out, mbLeftBlk + MB_BLK_OFF_LEFT[i], mbTopBlk + MB_BLK_OFF_TOP[i], I_16x16, I_16x16, ac[i], H264Const.totalZeros16, 1, 15, CoeffTransformer.zigzag4x4);
		}
	}
	...
	private void writeDC(int comp, int mbX, int mbY, BitWriter out, int qp, int mbLeftBlk, int mbTopBlk, int[] dc) {
		if (dc.length == 4) {
			...
			cavlc[comp].writeChrDCBlock(out, dc, H264Const.totalZeros8, 0, dc.length, new int[] { 0, 1, 2, 3, 4, 5, 6, 7 });
		} else {
			reorderDC4x4(dc);
			CoeffTransformer.quantizeDC4x4(dc, qp);
			CoeffTransformer.fvdDC4x4(dc);
			// TODO: calc here
			cavlc[comp].writeLumaDCBlock(out, mbLeftBlk, mbTopBlk, I_16x16, I_16x16, dc, H264Const.totalZeros16, 0, 16, CoeffTransformer.zigzag4x4);
		}
	}
	...
	private int[][] transform(Picture pic, int comp, int qp, int cw, int ch, int x, int y) {
		...
		for (int i = 0; i < ac.length; i++) {
			int[] coeff = ac[i];
			takeSubtract(pic.getPlaneData(comp), pic.getPlaneWidth(comp), pic.getPlaneHeight(comp), x + BLK_X[i], y + BLK_Y[i], coeff, dcc);
